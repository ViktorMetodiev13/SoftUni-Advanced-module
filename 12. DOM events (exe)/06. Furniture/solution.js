function solve() {
  ...
  function buy(e) {
    let checkboxex = Array.from(querySelectorAll('tbody input')).filter(c => c.checked);

    let bought = [];
    let price = 0;
    let decoration = 0;

    checkboxex.forEach(x => {
      // Take to levels above this element - the whole row
    ...
    output.textContent += `Average decoration factor: ${decoration / checkboxex.length}`;
  }
}